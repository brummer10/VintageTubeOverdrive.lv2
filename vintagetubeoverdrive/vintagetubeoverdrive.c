...
#define PLUGIN_UI_URI "urn:brummer:vintagetubeoverdrive_ui"
...
const char* plugin_set_name() {
    return "vintagetubeoverdrive"; //set plugin name to display on UI
}

void plugin_create_controller_widgets(X11_UI *ui, const char * plugin_uri, float scale) {
    set_costum_theme(ui->win);
    ui->widget[0] = add_lv2_image_toggle (ui->widget[0], ui->win, 2, "Bypass", ui, 61,  60, 60 * scale, 78 * scale);
    widget_get_png(ui->widget[0], LDVAR(switch_gbxc_png));

    ui->widget[1] = add_lv2_knob (ui->widget[1], ui->win, 3, "DRIVE", ui, 140,  60, 60 * scale, 80 * scale);
    ...
    ui->widget[3] = add_lv2_knob (ui->widget[3], ui->win, 5, "VOLUME", ui, 280,  60, 60 * scale, 80 * scale);
